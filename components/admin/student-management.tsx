...
export default function StudentManagement() {
  ...
  const fetchData = async () => {
    try {
      const [invitationsResult, studentsResult] = await Promise.all([
        supabase.from("student_invitations").select("*").eq("is_registered", false).order("created_at", { ascending: false }),
  ...
  const deleteActiveStudent = async (studentId: string, studentName: string) => {
                      ...
                      <TableCell>
                        <div className="flex space-x-1">
                          ...
                          <Button
                            size="sm"
                            variant="destructive"
                            onClick={() => deleteActiveStudent(student.id, student.full_name)}

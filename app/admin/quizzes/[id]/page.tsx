...
import { getQuizById, publishQuiz, unpublishQuiz } from '@/app/actions/content-management';
...
export default function AdminQuizBuilderPage() {
  ...
  const loadQuiz = async () => {
    try {
      // First try to load from database using the action
      const result = await getQuizById(quizId);
      if (result.success && result.data) {
        const quizData = result.data;
        const questions = quizData.quiz_data?.questions || [];
  ...
  const handlePublishQuiz = async () => {
    ...
    try {
      const result = await publishQuiz(quiz.id, userProfile.id);
      if (result.success) {
        setQuiz({ ...quiz, isPublished: true, publishedAt: new Date().toISOString() });
        toast({
  ...
  const handleUnpublishQuiz = async () => {
    ...
    try {
      ...
      if (result.success) {
        setQuiz({ ...quiz, isPublished: false, publishedAt: undefined });
        toast({
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          {quiz.isPublished ? (
            <Button variant="outline" onClick={handleUnpublishQuiz}>
              <XCircle className="h-4 w-4 mr-2" />
              Unpublish
            </Button>
          ) : (
            <Button variant="default" onClick={handlePublishQuiz}>

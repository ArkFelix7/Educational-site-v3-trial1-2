...
import { getQuizzesForManagement, deleteQuiz, publishQuiz, unpublishQuiz } from '@/app/actions/content-management';
...
export default function AdminQuizzesPage() {
  ...
  const handlePublishQuiz = async (quizId: string) => {
    console.log('Publish button clicked for quiz:', quizId);
    console.log('User profile:', userProfile);
    
    try {
      console.log('Calling publishQuiz function...');
      
      // Use userProfile.id if available, otherwise use a fallback
      const userId = userProfile?.id || 'admin-fallback';
      const result = await publishQuiz(quizId, userId);
      console.log('Publish result:', result);
      
      if (result.success) {
        toast({
          title: "Quiz published",
          description: "The quiz is now available to students.",
          variant: "default"
        });
        await loadQuizzes(); // Reload the list
      } else {
        ...
        toast({
          ...
          description: result.error || "Failed to publish the quiz.",
          variant: "destructive"
        });
      }
    } catch (error) {
  ...
  const handleUnpublishQuiz = async (quizId: string) => {
    console.log('Unpublish button clicked for quiz:', quizId);
    
    try {
      console.log('Calling unpublishQuiz function...');
      ...
      if (result.success) {
        toast({
          ...
          description: "The quiz is no longer available to students.",
          variant: "default"
        });
        await loadQuizzes(); // Reload the list
      } else {
        ...
        toast({
          ...
          description: result.error || "Failed to unpublish the quiz.",
          variant: "destructive"
        });
      }
    } catch (error) {
      ...
      toast({
        title: "Unpublish failed",
        description: "Failed to unpublish the quiz.",
        variant: "destructive"
      });
    }
  };
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <Button
                      ...
                      onClick={() => quiz.is_published ? handleUnpublishQuiz(quiz.id) : handlePublishQuiz(quiz.id)}
